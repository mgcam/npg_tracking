...

{
    ...
    is($r->command, q[perl -e 'use strict; use warnings; use Log::Log4perl qw(:easy);use npg::samplesheet::auto;BEGIN{ Log::Log4perl->easy_init($INFO); } npg::samplesheet::auto->new()->loop();'], 'command to run');
    is($r->daemon_name, 'npg_samplesheet_daemon', 'default daemon name');
    is($r->start(q[sf-1-1-01]), $bash_command . qq[daemon -i -r -a 10 -n npg_samplesheet_daemon --umask 002 -A 10 -L 10 -M 10 -o $log_dir/npg_samplesheet_daemon-] . q[sf-1-1-01-2013.log -- perl -e 'use strict; use warnings; use Log::Log4perl qw(:easy);use npg::samplesheet::auto;BEGIN{ Log::Log4perl->easy_init($INFO); } npg::samplesheet::auto->new()->loop();'] . $bash_error_command, 'start command');
